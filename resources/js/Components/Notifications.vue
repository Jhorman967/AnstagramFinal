<template>
    <div>
        <dropdown aling="rigth" width="80" overflow="overflow-y-auto" maxheight="300">
            <template #trigger>
            ...
            <template #content>
                <inertia-link v-if="$page.props.notifications.length > 0" v-for="(notification,index) in $page.props.notifications" :key="index" class="block hover:bg-gray-100 text-xs text-gray-600 flex items-center py-2 px-3">
        ...
        methods: {
            ...
            notification(){
                ...
                .notification((notification) =>{
                    this.$page.props.notification.push({
                        data:{
